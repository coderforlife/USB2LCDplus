...
/* Bootloader Version */
#define MINOR_VERSION   0x20    //Bootloader Version 2.0 for USB2LCD+ (5B)
#define MAJOR_VERSION   0x5B

/* The EEPROM locations for the serial number */
#define EEP_SER_NUM_0	0x0Eu
#define EEP_SER_NUM_1	0x0Fu

/* State Machine */
#define WAIT_FOR_CMD    0x00u    //Wait for Command packet
#define SENDING_RESP    0x01u    //Sending Response
...
#define OVER_HEAD   7           //Overhead: <CMD_CODE><LEN><ADDR:3><COUNT:2>
#define DATA_SIZE   (BOOT_EP_SIZE - OVER_HEAD)

typedef union _BOOT_DATA_PACKET
{
    byte bytes[BOOT_EP_SIZE];  //For Byte Access
    struct
    {
        enum
        {
