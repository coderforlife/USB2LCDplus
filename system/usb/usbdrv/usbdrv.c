...
void USBCheckBusStatus(void)
{
	...
	{											// host actively driving Vbus high?
        if(UCONbits.USBEN == 0u)                // Is the module off?
            USBModuleEnable();                  // When Vbus is high, it is okay
	}											// to turn on the USB module.
    else
    {
        if(UCONbits.USBEN == 1u)                // Is the module on?
...
********************************************************************/
    for(bTRNIFCount = 0u; bTRNIFCount < 4u; bTRNIFCount++)
    {
        if(UIRbits.TRNIF && UIEbits.TRNIE)
        {
             ...
             */
            if(USBCtrlEPService() == 0u) // If not an EP0 transaction, then clear TRNIF.
            {
                /*
...
void USBRemoteWakeup(void)
{
    ...
    if(usb_stat.RemoteWakeup == 1u)         // Check if RemoteWakeup function
...
void USBProtocolResetHandler(void)
{
    ...
    while(UIRbits.TRNIF == 1u)      // Flush any pending transactions
    {
        UIRbits.TRNIF = 0;
/********************************************************************
Bug Fix: August 14, 2007
