...
/* Detached is the state in which the device is not attached to the bus. When in the detached state a device should not have any pull-ups attached to either the D+ or D- line. */
#define DETACHED_STATE          0u

/* Attached is the state in which the device is attached to the bus but the hub/port that it is attached to is not yet configured.*/
#define ATTACHED_STATE          1u

/* Powered is the state in which the device is attached to the bus and the  hub/port that it is attached to is configured.*/
#define POWERED_STATE           2u

/* Default state is the state after the device receives a RESET command from the host.*/
#define DEFAULT_STATE           3u

/* Address pending state is not an official state of the USB defined states. This state is internally used to indicate that the device has received a SET_ADDRESS command but has
not received the STATUS stage of the transfer yet. The device is should not switch addresses until after the STATUS stage is complete. */
...
/* Configured is the state where the device has been fully enumerated and is operating on the bus. The device is now allowed to excute its application specific tasks.  It is also
